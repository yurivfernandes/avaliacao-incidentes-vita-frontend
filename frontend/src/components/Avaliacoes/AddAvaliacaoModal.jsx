...
function AddAvaliacaoModal({ ticket, onClose, onSuccess }) {
  ...
  useEffect(() => {
    const fetchPremissaAndCriterios = async () => {
      try {
        ...
        // Inicializar dados do formulário com base no ticket
        const initialData = {
          // Valores automáticos baseados no ticket
        ...
        if (premissaResponse.data.results && premissaResponse.data.results.length > 0) {
          ...
          const criteriosParaExibir = criteriosAtivos.filter(criterio => {
            if (criterio.field_service_now) {
              ...
              setFormData(prev => ({
                ...prev,
                [`criterio_${criterio.id}`]: valorCalculado
              }));
          ...
          criteriosParaExibir.forEach(criterio => {
            if (criterio.tipo === 'conversao') {
              ...
              setFormData(prev => ({
                ...prev,
                [`criterio_${criterio.id}`]: ''
              }));
            } else if (criterio.tipo === 'boolean') {
              setFormData(prev => ({
                ...prev,
                [`criterio_${criterio.id}`]: false
              }));
            } else if (criterio.tipo === 'integer') {
              setFormData(prev => ({
                ...prev,
                [`criterio_${criterio.id}`]: 0
              }));
            }
          });
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Transformar os dados do form no novo formato esperado pela API
      ...
      setError(err.response?.data?.message || 'Erro ao adicionar avaliação. Tente novamente.');
    } finally {
      setLoading(false);
    }
  };
            ...
            >
              ...
              {conversoes.map(conversao => (
                <option key={conversao.id} value={conversao.id}>
                  {conversao.nome} ({conversao.percentual}%)
                </option>
              ))}
            </select>
          </div>
      ...
      <div className="modal-container evaluation-modal">
        ...
        <div className="modal-content">
          <div className="ticket-info-compact">
            ...
            <div className="info-item">
              <span className="info-label">Sub-categoria:</span>
              <span className="info-value">{ticket.sub_categoria_falha}</span>
            </div>
            {ticket.dv_u_detalhe_sub_categoria_da_falha && (
              <div className="info-item">
                <span className="info-label">Detalhe:</span>
